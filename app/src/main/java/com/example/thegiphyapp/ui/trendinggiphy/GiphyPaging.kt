...
class GiphyPaging(val searchValue:String,val giphyServiceRepo: GiphyServiceRepository) : PagingSource<Int, GiphyData>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, GiphyData> {
        ...
        return try {
            if(searchValue.isNullOrEmpty()) {
                ...
                val searchGiphyData = giphyServiceRepo.searchGiphy(offSet = page.toString(),searchValue = searchValue)
                if(searchGiphyData.isEmpty()) {
                    val giphyTrendingList = giphyServiceRepo.giphyTrendingList(offSet = page.toString())
                    LoadResult.Page(
                            data = giphyTrendingList,
                            prevKey = if(page == 0) null else page - 10,
